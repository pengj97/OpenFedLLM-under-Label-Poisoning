...
for split_strategy in "${split_strategies[@]}"; do
    ...
    log_file=logs/sft_instruct/Qwen2.5-Math-7B/"$graph"_n="$num_clients"_b="$num_poisoned_clients"/"$attack"/"$fed_alg"_"$attack"_"$split_strategy".log
    ...
    CUDA_VISIBLE_DEVICES=$gpu nohup python main_dec_sft_instruction.py \
        --learning_rate $lr \
