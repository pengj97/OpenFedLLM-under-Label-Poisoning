...
for split_strategy in "${split_strategies[@]}"; do
    ...
    log_file=logs/sft/Llama-3.1-8B/"cen"_n="$num_clients"_b="$num_poisoned_clients"/"$attack"/"$fed_alg"_"$attack"_"$split_strategy".log

    mkdir -p "$(dirname "$log_file")"

    CUDA_VISIBLE_DEVICES=$gpu nohup python main_cen_sft_classification_llama.py \
        --learning_rate $lr \
