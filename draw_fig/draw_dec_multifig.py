...
colors = ['green', 'orange', 'blue', 'purple', 'olive', 'skyblue', 'gold', 'red',]
markers = ['s', 'v', '^',  'x',  'o', 'D', 'h',  '+',]

# colors = [ 'orange', 'blue', 'purple', 'olive', 'skyblue', 'gold', 'red',]
# markers = ['v', '^',  'x',  'o', 'D', 'h',  '+',]
...
pic_name = model + '_' + dataset + '_' + str(num_samples) + '_' + network + '_'  + attack +  f'_l{l}_b{batch_size}_lr{lr}'
# pic_name = model + '_' + dataset + '_'  + network + '_'  + attack + '_' + split_strategy + f'_l{l}_b{batch_size}_lr{lr}'


fig, axes = plt.subplots(2, len(split_strategies), figsize=(26, 14), sharex=True, sharey='row')
axes[0][0].set_ylabel('Accuracy', fontsize=FONTSIZE)
axes[1][0].set_ylabel('Loss', fontsize=FONTSIZE)
...
for i in range(len(split_strategies)):
    for j in range(2):
        axes[j][i].set_title(split_strategies[i][1] + f' ({dataset.upper()})', fontsize=FONTSIZE)
        # axes[j][i].set_title(split_strategies[i][1] + f' (STATE)', fontsize=FONTSIZE)
    ...
    for agg_index, (fed_alg, alg_name) in enumerate(fed_algs):
        ...
        if 'Baseline' in alg_name:
            ...
            file_path = f'{root}/{model}_r{r}a{a}_l{l}_{dataset}_{num_samples}/{network_baseline}/{split_strategies[i][0]}/{fed_alg}_i{max_steps}_b{batch_size}a{gradient_accumulation_steps}_None_lr{lr}'
        else:
        ...
        axes[0][i].plot(rounds, eval_accuracy_list, '-', label=alg_name, color=color, marker=marker, markevery=10, linewidth=4, markersize=10)
        axes[1][i].plot(rounds, eval_loss_list, '-', label=alg_name, color=color, marker=marker, markevery=10, linewidth=4, markersize=10)
        # axes[0].legend()

handles, labels = axes[0][0].get_legend_handles_labels()
fig.legend(handles, labels, loc='lower center', ncol=len(fed_algs), fontsize=FONTSIZE-20, markerscale=2)

plt.subplots_adjust(top=0.91, bottom=0.2, left=0.09, right=0.95, hspace=0.23, wspace=0.27)
