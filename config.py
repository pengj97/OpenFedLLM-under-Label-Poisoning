...
class FedArguments:
    ...
    num_poisoned_clients: Optional[int] = field(default=2, metadata={"help": "the number of poisoned clients in the set of sampled clients"})
    attack: Optional[str] = field(default=None, metadata={"help": "the type of attack to perform"})
    eval_interval: Optional[int] = field(default=10, metadata={"help": "the interval to evaluate the global model"})
    graph: Optional[str] = field(default=None, metadata={"help": "type of communication graph for decentralized aggregation (e.g. 'Complete','Line', etc). None means central server"})
...
class ScriptArguments:
    ...
    seed: Optional[int] = field(default=2025, metadata={"help": "the seed to use"})
...
if script_args.use_peft:
    peft_config = LoraConfig(
        ...
        "up_proj",
        "down_proj",
        # "score", # this setting is not recommended in practice, since the classification head usually has a small number of parameters
        ],
...
def save_config(script_args, fed_args, use_lora=True):
    dataset_name_split = os.path.basename(script_args.dataset_name)
    model_name = script_args.model_name_or_path.split("/")[-1]
    if fed_args.graph is not None:
        graph_name = fed_args.graph
    else:
        graph_name = "fed"
    
    if use_lora:
        output_dir = f"{script_args.output_dir}/{model_name}_lora_r{script_args.peft_lora_r}a{script_args.peft_lora_alpha}_l{script_args.seq_length}_{dataset_name_split}_{script_args.dataset_sample}/{graph_name}_n={fed_args.num_clients}_b={fed_args.num_poisoned_clients}/{fed_args.split_strategy}/{fed_args.fed_alg}_i{script_args.max_steps}_b{script_args.batch_size}a{script_args.gradient_accumulation_steps}_{fed_args.attack}_lr{script_args.learning_rate}"
    else:
        output_dir = f"{script_args.output_dir}/{model_name}_l{script_args.seq_length}_{dataset_name_split}_{script_args.dataset_sample}/{graph_name}_n={fed_args.num_clients}_b={fed_args.num_poisoned_clients}/{fed_args.split_strategy}/{fed_args.fed_alg}_i{script_args.max_steps}_b{script_args.batch_size}a{script_args.gradient_accumulation_steps}_{fed_args.attack}_lr{script_args.learning_rate}"

    if not os.path.exists(output_dir):
        print(f"[INFO] Saving config to: {output_dir}")
